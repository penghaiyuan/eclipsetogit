$(function () {
    ...
    $("#btn_login").click(function () {
        ...
        $.ajax({
            url: "/hwshop/admin/login/doLogin",
            type:"post",
...
function initialCookie() {
    ...
    if(document.cookie.length>0) {
        ...
        if(url !== null) {
            ...
            $("#div_background").css("background-image", "url(/hwshop/res/images/admin/loginPage/background-1.jpg)");
        }
        if(username !== null){
...
function getUserProfilePicture(username) {
    if(username !== null && username !== ""){
        $.getJSON("/hwshop/admin/login/profile_picture",{"username":username},function (data) {
            if(data.success){
                if(data.srcString !== null){
                    $("#img_profile_picture").attr("src", "/hwshop/res/images/item/adminProfilePicture/" + data.srcString);
                    return true;
                }
            }
        });
    }
    $("#img_profile_picture").attr("src","/hwshop/res/images/admin/loginPage/default_profile_picture-128x128.png");
}